export const POST_CREATE_REQUEST = 'POST_CREATE_REQUEST';
export const POST_CREATE_SUCCESS = 'POST_CREATE_SUCCESS';
export const POST_CREATE_FAIL = 'POST_CREATE_FAIL';

export const POSTS_FETCH_REQUEST = 'POSTS_FETCH_REQUEST';
export const POSTS_FETCH_SUCCESS = 'POSTS_FETCH_SUCCESS';
export const POSTS_FETCH_FAIL = 'POSTS_FETCH_FAIL';

export const USER_POSTS_FETCH_REQUEST = 'USER_POSTS_FETCH_REQUEST';
export const USER_POSTS_FETCH_SUCCESS = 'USER_POSTS_FETCH_SUCCESS';
export const USER_POSTS_FETCH_FAIL = 'USER_POSTS_FETCH_FAIL';

export const POST_FETCH_REQUEST = 'POST_FETCH_REQUEST';
export const POST_FETCH_SUCCESS = 'POST_FETCH_SUCCESS';
export const POST_FETCH_FAIL = 'POST_FETCH_FAIL';

export const POST_LIKE_REQUEST = 'POST_LIKE_REQUEST';
export const POST_LIKE_SUCCESS = 'POST_LIKE_SUCCESS';
export const POST_LIKE_FAIL = 'POST_LIKE_FAIL';

export const POST_COMMENT_REQUEST = 'POST_COMMENT_REQUEST';
export const POST_COMMENT_SUCCESS = 'POST_COMMENT_SUCCESS';
export const POST_COMMENT_FAIL = 'POST_COMMENT_FAIL';

export const POST_LIKE_INSIDE_REQUEST = 'POST_LIKE_INSIDE_REQUEST';
export const POST_LIKE_INSIDE_SUCCESS = 'POST_LIKE_INSIDE_SUCCESS';
export const POST_LIKE_INSIDE_FAIL = 'POST_LIKE_INSIDE_FAIL';

export const POST_COMMENT_INSIDE_REQUEST = 'POST_COMMENT_INSIDE_REQUEST';
export const POST_COMMENT_INSIDE_SUCCESS = 'POST_COMMENT_INSIDE_SUCCESS';
export const POST_COMMENT_INSIDE_FAIL = 'POST_COMMENT_INSIDE_FAIL';
